#!/usr/bin/env bash
# 一期周报全流程：capture(可选) → revisit → rank → pick（此时才下载当期图）→ material → flatten
# → generate(写稿) → assemble(排版) → baoyu md→html → inline_images base64。
#
# 环境变量（可覆盖，均有默认值）：
#   ISSUE          期号（默认 1）
#   DB             SQLite 数据库路径（默认 data/pubpeer.db）
#   WINDOW         回访/打分相对窗口 "D1 D2"（默认 "10 3"，引用时展开成两个参数，勿加引号改语义）
