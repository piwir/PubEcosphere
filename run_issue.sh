...
# → site-json（自动归档本期到 src/site/src/data/site.json，公众号链接除外）。
#
# 环境变量（可覆盖，均有默认值）：
...
echo "==> [5/9] material（合并图写回 $OUT_ROOT/pub/）"
...
export WEEK_START
echo "==> [6/9] generate --assemble（提取+写稿+排版 → $WEEKLY/）"
...
echo "==> [7.3/9] polish_html（上下标 + 页脚小字 + GitHub 链接修复）"
python -m llm.polish_html "$WEEKLY/$ISSUE.html"

# 7) 图片 base64 内联（微信手动粘贴专用；同时清掉转换器 data-local-path 绝对路径残留）
echo "==> [8/9] inline_images（→ $WEEKLY/$ISSUE-base64.html）"
python -m llm.inline_images "$WEEKLY/$ISSUE.html"

# 9) 官网归档：把本期期号/日期/素材窗口/归档条目写进 site.json（公众号链接除外；
#    发布后在 issues[].wechatUrl 补链接再跑一次即同步首页 CTA）。-1 测试期不写站点数据。
if [[ "$ISSUE" != "-1" ]]; then
    echo "==> [9/9] site-json 归档（第 $ISSUE 期）"
    python -m sitejson sync --issue "$ISSUE"
fi
...
if [[ "$ISSUE" != "-1" ]]; then
    echo "    官网归档    src/site/src/data/site.json（公众号链接发布后补 issues[].wechatUrl）"
fi
