...
| 形态 | 载体 | 适用场景 |
| --- | --- | --- |
| MCP server | `mcp_server.py`（`mcp.sh` 启动） | 定时 / 无头客户端：16 个确定性工具转发 |
...
- 周报 13 工具：`status` / `preflight` / `capture` / `revisit` / `rank` / `pick` / `material` / `generate` / `assemble` / `md2html` / `polish_html` / `inline_images` / `run_issue`。
